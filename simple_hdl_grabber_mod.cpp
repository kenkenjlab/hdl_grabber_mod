...
public:
	...
	SimpleHDLViewer (HDLGrabber& grabber, pcl::visualization::PointCloudColorHandler<PointType> &handler) :
	...
	void run () {
		...
		*/
		...
		while (!cloud_viewer_->wasStopped ()) {
			CloudConstPtr cloud;
			if(grabber_.isSyncMode()) { grabber_.acquire(); }
...
private:
	...
	IplImage *iplimage_;
	HDLGrabber& grabber_;
	boost::mutex cloud_mutex_, image_mutex_;
...
int main (int argc, char ** argv) {
	...
	pcl::console::parse_argument (argc, argv, "-upsample", upsample_max_sq_dist_threshold);
	bool average_intensity = pcl::console::find_argument (argc, argv, "-average_intensity") != -1;

	// Start grabber
	HDLGrabber grabber (hdlCalibration, pcapFile);
	if(sync) { grabber.setSyncMode(); }
	if(upsample) {
		grabber.setUpsampleMode(average_intensity);
		grabber.setUpsampleMaxSquaredDistanceThreshold(upsample_max_sq_dist_threshold);
	}
